...
def convert_row(row, schema):
    ...
    for key, value in row.items():
        ...
        LOGGER.debug("Converting %s value %s to %s", key, value, declared_types)
...
def pick_datatype(counts, prefer_number_vs_integer=False):
    ...
    if len(counts) == 1:
        if counts.get("integer", 0) > 0:
            ...
            LOGGER.warning("Unexpected data type encountered in histogram %s. Defaulting type to String.", counts)

    elif len(counts) == 2 and counts.get("integer", 0) > 0 and counts.get("number", 0) > 0:
        to_return = "number"
    else:
        LOGGER.debug("Ambiguous combination of data type detected: %s. Defaulting type to String.", counts)
...
def generate_schema(samples, prefer_number_vs_integer=False, prefer_schema_as_string=False):
    ...
    for key, value in counts.items():
        if prefer_schema_as_string:
            to_return[key] = {"type": ["null", "string"]}
        else:
